...
# -------------------------------------------------------------------
# fetch_latest_image: scrape SourceForge (improved)
# Arguments: $1 = base_path, $2 = variant
# Returns: full download URL or empty
# -------------------------------------------------------------------
fetch_latest_image() {
    local base_path="$1"
...
# Copy Waydroid Python modules into lib/waydroid (not bin/)
cp -r /usr/lib/waydroid ./AppDir/lib/
# The waydroid.py is now at ./AppDir/lib/waydroid/waydroid.py

# -------------------------------------------------------------------
# Create wrapper for waydroid BEFORE quick-sharun runs
# -------------------------------------------------------------------
...
if [ "$1" = "init" ]; then
    shift
    exec python3 "$APPDIR/lib/waydroid/waydroid.py" init -i "$IMAGE_DIR" "$@"
else
    exec python3 "$APPDIR/lib/waydroid/waydroid.py" "$@"
fi
EOF
chmod +x ./AppDir/bin/waydroid

# -------------------------------------------------------------------
# Deploy additional binaries – pass only files, not directories
# (we list each executable explicitly)
# -------------------------------------------------------------------
quick-sharun \
    ./AppDir/bin/waydroid \
    /usr/bin/nft \
...
# Clean up unwanted files in dbus-1 and polkit-1
find ./AppDir/share/dbus-1 ./AppDir/share/polkit-1 -type f ! -name '*waydro*' -delete

# -------------------------------------------------------------------
# Set environment and create AppImage
# -------------------------------------------------------------------
